@extends('layout.layout')
    ...
    <style>
        ...
        #iaInput:focus { border: none !important; outline: none !important; box-shadow: none !important; -webkit-appearance: none; appearance: none; }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function(){
            ...
            function getHistory(){
                try{ return JSON.parse(localStorage.getItem('iaChatHistory')||'[]'); }
                catch(e){ return []; }
            }
            function pushHistory(role, payload){
                try{
                    const h = getHistory();
                    let entry = { role, ts: new Date().toISOString() };
                    if (typeof payload === 'string') {
                        entry.type = 'text';
                        entry.text = payload;
                    } else if (payload && payload.type === 'table') {
                        entry.type = 'table';
                        entry.title = payload.title || '';
                        entry.items = payload.items || [];
                        entry.cols = payload.cols || [];
                    } else if (payload && payload.type === 'chart') {
                        entry.type = 'chart';
                        entry.labels = payload.labels || [];
                        entry.actual = payload.actual || [];
                        entry.title = payload.title || '';
                    } else {
                        entry.type = 'text';
                        entry.text = String(payload || '');
                    }
                    h.push(entry);
                    localStorage.setItem('iaChatHistory', JSON.stringify(h));
                }catch(e){}
            }
            ...
            function addAiList(thinkingEl, title, items, cols) {
                ...
                chat.appendChild(el);
                scrollToBottom();
                pushHistory('ai', { type: 'table', title, items, cols });
            }
                    ...
                    const imgHtml = `
                        <div style="display:flex; flex-direction:column; gap:10px;">
                            ...
                            <div class="ia-panel-graf">
                                <img src="data:image/png;base64,${data.plot_png_base64}" style="width:100%; border-radius:8px;">
                            </div>
            ...
            clearBtn.addEventListener('click', async () => {
                if (typeof Swal !== 'undefined') {
                    const r = await Swal.fire({
                        ...
                        text: '¿Deseas eliminar todas las conversaciones del asistente?',
                        icon: 'warning',
            ...
            (function renderHistory(){
                ...
                if(h.length > 0) {
                    h.forEach(m => {
                        if (m.type === 'table') {
                            let tableHtml = `<div style="font-weight:700; margin-bottom:0.8rem; color:#4f46e5">${m.title || ''}</div>`;
                            tableHtml += `<div class="ia-table-container"><table class="ia-table"><thead><tr>`;
                            (m.cols || []).forEach(col => tableHtml += `<th>${col}</th>`);
                            tableHtml += `</tr></thead><tbody>`;
                            if(!m.items || m.items.length === 0) {
                                tableHtml += `<tr><td colspan="${(m.cols||[]).length}" style="text-align:center">No hay datos disponibles.</td></tr>`;
                            } else {
                                (m.items || []).forEach(row => {
                                    tableHtml += `<tr>`;
                                    (m.cols || []).forEach(col => {
                            ...
                            const content = formatRich(m.text || '');
                            const el = createMessageElement(m.role === 'user' ? 'user' : 'ai', content);
                            chat.appendChild(el);
                        }
                    });
                    scrollToBottom();
