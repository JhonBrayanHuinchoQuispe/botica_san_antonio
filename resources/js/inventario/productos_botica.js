document.addEventListener('DOMContentLoaded', () => {
  ...
  function presentacionesBadge(prod) {
    ...
    if (total === 0) {
      return `<div class="presentacion-badge empty">
        <span class="text-gray-500 text-sm">Sin presentaciones</span>
      </div>`;
    }
    
    if (total === 1) {
      const pres = presentaciones[0];
      return `<div class="presentacion-badge single">
        <div class="flex flex-col">
          <span class="font-medium text-gray-800">${pres.nombre_presentacion}</span>
          <span class="text-xs text-gray-600">${pres.unidades_por_presentacion} ${pres.unidades_por_presentacion === 1 ? 'unidad' : 'unidades'}</span>
        </div>
      </div>`;
    }
    
    // Para múltiples presentaciones, hacemos clicable y mostramos modal
    const primerasPresentaciones = presentaciones.slice(0, 2).map(p => 
      `<span class="text-xs text-gray-700">${p.nombre_presentacion} (${p.unidades_por_presentacion})</span>`
    ).join(', ');
    
    return `<div class="presentacion-badge multiple cursor-pointer hover:bg-gray-50 transition-colors" onclick="verPresentacionesDeProducto('${prod.id}')">
      <div class="flex flex-col gap-1">
        <span class="font-semibold text-blue-600">${total} Presentaciones</span>
        <span class="text-xs text-gray-600">${primerasPresentaciones}${total > 2 ? '...' : ''}</span>
      </div>
    </div>`;
  }

  // Función global para ver presentaciones
  window.verPresentacionesDeProducto = function(id) {
    ...
    // Generar tabla de presentaciones
    const rows = presentaciones.map(pres => {
      ...
      return `
        <tr class="hover:bg-blue-50 transition-colors">
          <td class="px-4 py-3 font-medium text-gray-800">${pres.nombre_presentacion}</td>
          <td class="px-4 py-3 text-center text-gray-700">${pres.unidades_por_presentacion}</td>
          <td class="px-4 py-3 text-center font-semibold text-green-600">${precio}</td>
        </tr>
      `;
    }).join('');
    
    Swal.fire({
      title: `<div style="font-size: 1.25rem; font-weight: 600; color: #1f2937;">
                <i class="fas fa-boxes" style="color: #6366f1; margin-right: 8px;"></i>
                Presentaciones de ${prod.nombre}
              </div>`,
      html: `
        <div style="margin-top: 1rem;">
          <table style="width: 100%; border-collapse: collapse;">
            <thead style="background-color: #f9fafb; border-bottom: 2px solid #e5e7eb;">
              <tr>
                <th style="padding: 12px; text-align: left; font-weight: 600; color: #374151;">Presentación</th>
                <th style="padding: 12px; text-align: center; font-weight: 600; color: #374151;">Unidades</th>
                <th style="padding: 12px; text-align: center; font-weight: 600; color: #374151;">Precio Venta</th>
              </tr>
            </thead>
  ...
  async function abrirDetalles(id, loteSeleccionado = null) {
    try {
      ...
      // Renderizar presentaciones del producto
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-lg overflow-hidden">
            <table class="w-full text-sm">
              <thead class="bg-indigo-50">
                <tr>
                  <th class="px-4 py-2 text-left font-semibold text-gray-700">Presentación</th>
                  <th class="px-4 py-2 text-center font-semibold text-gray-700">Unidades</th>
                  <th class="px-4 py-2 text-center font-semibold text-gray-700">Precio Venta</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200">
                ${p.presentaciones.map(pres => `
                  <tr class="hover:bg-indigo-50 transition-colors">
                    <td class="px-4 py-3 font-medium text-gray-800">${pres.nombre_presentacion}</td>
                    <td class="px-4 py-3 text-center text-gray-700">${pres.unidades_por_presentacion} ${pres.unidades_por_presentacion === 1 ? 'unidad' : 'unidades'}</td>
                    <td class="px-4 py-3 text-center font-semibold text-green-600">S/ ${parseFloat(pres.precio_venta_presentacion || 0).toFixed(2)}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
...
async function abrirModalEdicion(productId, loteSeleccionado = null) {
  ...
  if (typeof window.abrirModalEditarProducto === 'function') {
    ...
    // Cargar presentaciones después de abrir el modal
    if (typeof window.loadExistingPresentaciones === 'function') {
      await window.loadExistingPresentaciones(p.id);
    }
  } else {
    ...
    if (typeof window.loadExistingPresentaciones === 'function') {
      window.loadExistingPresentaciones(p.id);
    }
