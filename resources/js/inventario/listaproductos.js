document.addEventListener("DOMContentLoaded", function() {
    ...
    async function guardarNuevoProducto() {
        ...
        try {
            ...
            // Agregar presentaciones si la función existe
            if (typeof window.getPresentacionesData === 'function') {
                console.log('📦 Obteniendo datos de presentaciones...');
                const presentaciones = window.getPresentacionesData();
                console.log('📦 Presentaciones obtenidas:', presentaciones);
                
                // Agregar presentaciones al FormData
                let index = 0;
                Object.values(presentaciones).forEach((pres) => {
                    formData.append(`presentaciones[${index}][id]`, pres.id || '');
                    formData.append(`presentaciones[${index}][nombre_presentacion]`, pres.nombre_presentacion);
                    formData.append(`presentaciones[${index}][unidades_por_presentacion]`, pres.unidades_por_presentacion);
                    formData.append(`presentaciones[${index}][precio_venta_presentacion]`, pres.precio_venta_presentacion || '');
                    index++;
                });
                console.log('✅ Total presentaciones agregadas:', index);
            } else {
                console.warn('⚠️ Función window.getPresentacionesData no disponible');
            }
    ...
    async function guardarEdicionProducto() {
        ...
        // Agregar presentaciones si la función existe
        if (typeof window.getPresentacionesData === 'function') {
            console.log('📦 Obteniendo datos de presentaciones para editar...');
            const presentaciones = window.getPresentacionesData();
            console.log('📦 Presentaciones obtenidas:', presentaciones);
            ...
            console.log('✅ Total presentaciones agregadas para editar:', index);
        } else {
            console.warn('⚠️ Función window.getPresentacionesData no disponible');
        }
