...
class POSProfesional {
    ...
    crearTarjetaProducto(producto) {
        ...
        if (producto.stock_actual > 0 && !esVencido) {
            card.addEventListener('click', () => {
                // Si tiene múltiples presentaciones, mostrar selector
    ...
    mostrarSelectorPresentaciones(producto) {
        const presentaciones = producto.presentaciones || [];
        ...
        const filasPresentaciones = presentaciones.map((pres, index) => {
            ...
            return `
                <tr class="presentacion-row-modal" data-presentacion-id="${pres.id}" data-presentacion-nombre="${nombre}" data-presentacion-precio="${precio}">
        ...
        setTimeout(() => {
            ...
            filas.forEach(fila => {
                fila.addEventListener('click', () => {
                    ...
                    this.agregarAlCarritoConPresentacion(producto.id, presentacionId, presentacionNombre, precio);
                });
            });
        }, 50);
    ...
    async agregarAlCarritoConPresentacion(productoId, presentacionId, presentacionNombre, precio) {
        ...
        // Buscar si ya existe en el carrito con la misma presentación
        const itemExistente = this.carrito.find(item => 
            item.producto_id == productoId && item.presentacion_id == presentacionId
        );
